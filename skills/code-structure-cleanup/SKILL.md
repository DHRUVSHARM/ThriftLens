...
description: Use only when explicitly requested after an AI-built feature works but the code has duplicated mechanics, repeated API calls, or messy structure. Guides a focused cleanup pass that extracts reusable service-layer modules without changing behavior.
version: 1.0.0
...
# Code Structure Cleanup After a Feature
...
- The user explicitly asks to run `code-structure-cleanup`.
